...
import fakesmtp.client.utilitiy.MyUtility;

import javax.swing.JOptionPane;
import javax.swing.JTextArea;
import javax.swing.JButton;

import fakesmtp.client.model.Mail;
import fakesmtp.client.controller.PopController;
import fakesmtp.client.controller.SmtpController;
...
public class MainFrame extends JFrame {
	...
	public MainFrame(String addr, String pass, String smtp_addr, String pop_addr,LoginDialog loginDialog) {
		...
		tabbedPane.addTab("收信",  panel_box);
	...
	private void initPop() {
		...
		Mail mail;
		if (popController.mailList==null){
			return;
		}
		for (String email:popController.mailList){
			String num = extractNumericPart(email);
			mail = new Mail(popController.getItemString(num+""));
			mail.setTag(num);
			mails.add(mail);
		}
...
//							.get(rowIndex).getTo_list().get(0) : "匿名";
	...
	public static String extractNumericPart(String input) {
		...
		if (matcher.find()) {
			return matcher.group(); // 返回匹配到的数字部分
		} else {
			return ""; // 如果没有找到数字部分，则返回空字符串或者其他适当的默认值
		}
	}
...
//		if(mails.get(curMailIndex).getTo_list().size() == 0) JOptionPane.showMessageDialog(MainFrame.this, "该用户匿名，无法回复！");
		if(mails.get(curMailIndex).getTo_list().size() == 0) JOptionPane.showMessageDialog(MainFrame.this, "该用户匿名，无法回复！");
			...
			addrs_text.setText(mails.get(curMailIndex).getFrom());
			subject_text.setText("回复邮件:"+mails.get(curMailIndex).getSubject());//产生叠加
		}
	}

	protected void doDelete() {
		long num  = Long.parseLong(mails.get(curMailIndex).getTag());
		if(popController.deleteItem(num+"")){
